...
# Description: This script trains an inverse-variance weighted XGBoost model,
#              extracts SHAP values, performs permutation/bootstrap validation,
...
# Ensure validation.py is in the same directory
try:
...
# ==============================================================================
# USERS: Set base_dir to your project folder or use relative path
base_dir = "./"
...
# Filter criteria matching the primary analysis
df = df[df['Category'] != 'Overall'].copy()
df = df[(df['HR'] > 0) & (df['Lower_CI'] > 0) & (df['Upper_CI'] > 0)].copy()

# Calculate variance and statistical weights
...
X.columns = X.columns.str.replace('<', 'lt_')
X.columns = X.columns.str.replace('>', 'gt_')
...
# ==============================================================================
# 5. Figure 2: SHAP Summary
# ==============================================================================
print("Generating Figure 2 and data...")
plt.figure(figsize=(10, 5))
shap.summary_plot(shap_values, X_display, show=False, max_display=15, cmap=nature_cmap, plot_size=(10, 5.5))
...
df_fig2_summary = pd.DataFrame({'Feature': clean_feature_names, 'Mean_Abs_SHAP': mean_abs_shap})
df_fig2_summary = df_fig2_summary.sort_values(by='Mean_Abs_SHAP', ascending=False)
df_fig2_summary.to_csv(os.path.join(out_dir, "Figure2_Data_MeanAbs_SHAP.csv"), index=False)
df_fig2_raw = pd.DataFrame(shap_values.values, columns=clean_feature_names)
df_fig2_raw.to_csv(os.path.join(out_dir, "Figure2_Data_Raw_SHAP.csv"), index=False)

# ==============================================================================
# 6. Figure S1: LOTO Sensitivity Heatmap
# ==============================================================================
...
sns.heatmap(rank_df_top_sens, annot=True, cmap=sns.light_palette(npg_red, reverse=True, as_cmap=True),
            linewidths=1.5, linecolor='white', fmt="g", vmin=1, vmax=10)
...
# ==============================================================================
# 7. Figure S2: Permutation Test (1x3 Panel)
# ==============================================================================
print("Executing Permutation Test...")
target_features = ['Subgroup_gt_=50%', 'Subgroup_lt_1%', 'Subgroup_T4']

true_importance, permuted_importances, p_vals = run_permutation_test(
    X=X, y=y, params=best_params, n_permutations=1000
)

# Export all features' P-values
pd.DataFrame(list(p_vals.items()), columns=['Feature', 'P_value']).sort_values('P_value').to_csv(
    os.path.join(out_dir, "All_Features_P_Values.csv"), index=False
)
...
for idx, feature in enumerate(target_features):
    ...
    ax.hist(permuted_importances[:, feat_idx], bins=20, alpha=0.8, color=color_null,
    ...
    ax.annotate(annot_text, xy=(0.95, 0.95), xycoords='axes fraction',
...
out_pdf_figS2 = os.path.join(out_dir, "FigureS2_Permutation_Panel_1x3.pdf")
plt.savefig(out_pdf_figS2, format='pdf', bbox_inches='tight')

pd.DataFrame(figS2_summary_data).to_csv(os.path.join(out_dir, "FigureS2_Data_Permutation_Summary.csv"), index=False)
pd.DataFrame(figS2_perm_dist_data).to_csv(os.path.join(out_dir, "FigureS2_Data_Permutation_Distributions.csv"), index=False)
...
mean_shap = true_importance

# ==============================================================================
# 9. Figure S4: PD-L1 Gradient Plot
# ==============================================================================
print("Generating Figure S4 (PD-L1 Gradient) and data...")
pdl1_features = ['Subgroup_lt_1%', 'Subgroup_1-49%', 'Subgroup_gt_=50%']
pdl1_names = ['PD-L1 < 1%', 'PD-L1 1-49%', 'PD-L1 ≥ 50%']
pdl1_means = [mean_shap[list(X.columns).index(f)] if f in X.columns else 0 for f in pdl1_features]
...
plt.errorbar(pdl1_names, pdl1_means, yerr=pdl1_errs, fmt='-o', color=npg_red,
             ecolor=npg_light, elinewidth=3, capsize=5, markersize=10, linewidth=2)
...
# ==============================================================================
print("Generating Figure S5 (Calibration) and data...")
...
plt.ylabel("XGBoost predicted $\\ln(\\mathrm{HR})$", fontsize=11)
plt.text(0.05, 0.95, f"$R^2$ = {r2:.3f}\nRMSE = {rmse:.3f}\nSlope = {slope:.2f}", transform=plt.gca().transAxes,
         va='top', bbox=dict(boxstyle='round', facecolor=npg_light, alpha=0.8, edgecolor='none'))
...
model_en = ElasticNet(alpha=0.01, l1_ratio=0.5, random_state=GLOBAL_SEED, max_iter=10000).fit(X, y, sample_weight=weights)
y_pred_en = model_en.predict(X)
model_rf = RandomForestRegressor(n_estimators=300, max_depth=3, random_state=GLOBAL_SEED, n_jobs=-1).fit(X, y, sample_weight=weights)
